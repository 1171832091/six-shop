<template>
  <div>
    ...
    <div>
      <!-- 优惠券单元格 -->
      <van-coupon-cell @click="showList = true" :coupons="coupons" :chosen-coupon="chosenCoupon" />
      <!-- 优惠券列表 -->
    ...
    <div class="payment-bg">
      ...
      <div class="button" @click="sub">立即支付</div>
    </div>
  </div>
</template>
<script>
...
Vue.use(NavBar).use(AddressList).use(CouponCell).use(CouponList).use(popup);
...
export default {
  ...
  created() {
      this.list[0].name = localStorage.getItem('Dname')
      this.list[0].tel = localStorage.getItem('Dtel')
      this.list[0].address = localStorage.getItem('Daddress')
  },
  methods: {
    onClickLeft() {
      this.$router.go(-1);
